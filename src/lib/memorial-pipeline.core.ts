...
function buildPrompt(m: Record<string, unknown>, lang: "en" | "es"): string {
  ...
  push("Where they spent their life", m.location as string);
  push("What they did", m.occupation as string);
  push("Three personality words", m.personality_words as string);
  push("What they loved most", m.loves as string);
  push("Strongest memory the writer has of them", m.strongest_memory as string);
  push("Inside detail only people close to them know", m.insider_detail as string);
  push("A phrase or saying they always used", m.catchphrase as string);
  push("People to mention by name", m.named_people as string);
...
async function generateNarrative(client: Anthropic, m: Record<string, unknown>, lang: "en" | "es"): Promise<string> {
...
export async function runMemorialPipeline(memorialId: string): Promise<{
  ...
  try {
    ...
    // Don't fail the whole pipeline for QR issues
  }
